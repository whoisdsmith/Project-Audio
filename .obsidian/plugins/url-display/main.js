...
var PARTITION = /\[(?<alias>.*?)\]\((?<link>.+?)\)/g;
var SPECIAL = /(\?target=){1}(?<target>https?.*)/g;
var EXCLUDE = /https?:\/\/.*?(apng|avif|bmp|gif|ico|jpeg|jpg|png|svg|tif|tiff|webp)/g;
  ...
  constructor(leaf, plugin) {
    ...
    this.updateDisplay = () => {
      ...
      if (!this.plugin.isExtracting && !this.plugin.activeNotehaveUrl) {
        container.createEl("p", { text: "No valid URL found in this note \u{1F604}" });
      }
...
// src/main.ts
var _UrlDisplayPlugin = class extends import_obsidian4.Plugin {
  constructor() {
    super(...arguments);
    ...
    this.activateView = async () => {
      if (this.app.workspace.getActiveViewOfType(import_obsidian4.MarkdownView)) {
        ...
        new import_obsidian4.Notice("Move focus into a note \u{1F609}");
      }
    };
    this.initState = () => {
      this.isExtracting = void 0;
      this.isParsing = void 0;
      this.activeNotehaveUrl = void 0;
      this.activeNoteUrlParse = null;
    };
    this.updateUrl = (0, import_obsidian4.debounce)(async () => {
    ...
    this.parseUrl = async (activeNoteUrl) => {
      this.isParsing = true;
      this.updateView();
    ...
    this.convertToObject = (activeNoteUrl) => {
      let UrlObject = [];
    ...
    this.cleanUrl = (UrlObject) => {
      if (this.settings.deduplicateUrls) {
        UrlObject = _UrlDisplayPlugin.deduplicateObjArrByUniId(UrlObject, "link");
      }
  ...
  async onload() {
    ...
    this.addRibbonIcon("external-link", "Open URL Dispaly", (evt) => {
      this.openOrClosePane();
    });
    this.addCommand({
...
var UrlDisplayPlugin = _UrlDisplayPlugin;
UrlDisplayPlugin.deduplicateObjArrByUniId = (arr, uniId) => {
